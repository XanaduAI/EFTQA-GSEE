...
def trend_filter(x, y, y_err=None, alpha_1=0.0,
    ...
    exp_y = 1
    if sigma>0:
        exp_y = np.sum(np.exp(-(y[1:]-y[:-1])/sigma) )

    derv_1 = first_derv_nes_cvxpy(x, result['base_model'])

    # the regularization
    reg_sum, regs = get_reg(x, result['base_model'], derv_1, l_norm, alpha_1, alpha_2, exp_y,
                            linear_deviations=linear_deviations)
...
def get_reg(x, base_model, derv_1, l_norm, alpha_1, alpha_2, exp_y, linear_deviations=None):
    """
    ...
    reg_1 = alpha_1 * norm(derv_1) * exp_y
