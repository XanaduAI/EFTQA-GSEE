...
def main(number, initial_state, model, step):
    ...
    moments = np.load(
        "results/{}/moments_{}_{}.npy".format(number, initial_state, step), allow_pickle=True
    )
    ...
    for ddd, d in enumerate(D_space):
        ...
        for _i, N in enumerate(tqdm(sample_space)):
            ...
            for run in range(10):  # do statistics over sampling
                ...
                np.save(
                    "results/{}/acdf_{}_{}_{}_{}_{}.npy".format(
                        number, initial_state, step, d, N, run
                    ),
                    acdf,
                )
                np.save(
                    "results/{}/energies_{}_{}_{}_{}_{}.npy".format(
                        number, initial_state, step, d, N, run
                    ),
                    energies,
                )
                ...
                plt.legend()
                plt.savefig(
                    "results/{}/acdf_{}_{}_{}_{}.png".format(number, initial_state, step, d, N)
                )
                # plt.show()
                ...
                np.save(
                    "results/{}/grad_acdf_{}_{}_{}_{}_{}.npy".format(
                        number, initial_state, step, d, N, run
                    ),
                    grad_acdf,
                )
                np.save(
                    "results/{}/grad_energies_{}_{}_{}_{}_{}.npy".format(
                        number, initial_state, step, d, N, _i
                    ),
                    new_energies,
                )
    np.save(
        "results/{}/guess_energies_rpt_{}_{}.npy".format(number, initial_state, step), new_energies
    )
